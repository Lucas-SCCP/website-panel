...
export default function ButtonElement({ element }: { element: ElementType }) {
  ...
  const handleSelectType = (selectedType: string) => {
    console.log('Selected button type:', selectedType);
    setType(selectedType);
  };

  const handleSelectAction = (selectedAction: string) => {
    console.log('Selected button action:', selectedAction);
    setAction(selectedAction);
  };

  const handleSelectSuccessMessageId = (id: number) => {
    console.log('Selected success message ID:', id);
    setSuccessMessageId(id);
  };

  const handleSelectSuccessActionId = (id: number) => {
    console.log('Selected success action ID:', id);
    ...
    <Row>
      ...
      <Col lg={12}>
        <Form.Group className="mb-3" controlId="pageName">
          ...
          <Form.Control type="text" placeholder="Digite o texto de carregamento do botão" value={element.properties.loadingMessage} />
        </Form.Group>
      </Col>
      <Col lg={6}>
        <Form.Group className="mb-3" controlId="pageName">
